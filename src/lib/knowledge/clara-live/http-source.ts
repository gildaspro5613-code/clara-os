...
export class ClaraLiveHttpKnowledgeSource implements ClaraLiveKnowledgeSource {
  ...
  public async search(query: ClaraLiveKnowledgeQuery): Promise<readonly ClaraLiveKnowledgeReference[]> {
    ...
    if (!response.ok) throw new Error(`Clara Live Knowledge request failed with status ${response.status}`);
    const payload: unknown = await response.json();
    if (!Array.isArray(payload)) throw new Error("Clara Live Knowledge response must be an array");

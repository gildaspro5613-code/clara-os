...
export type MakeWebhookErrorCode = "INVALID_URL" | "TIMEOUT" | "NETWORK_ERROR" | "HTTP_ERROR";
...
function readExecutionMetadata(response: Response, data: unknown): {
  ...
  async execute(configuration: MakeScenarioConfiguration, invocation: MakeScenarioInvocation): Promise<MakeScenarioExecutionResult> {
    ...
    try {
      response = await this.fetcher(url, {
        method: "POST",
        headers: { "content-type": "application/json", ...(configuration.headers ?? {}) },

...
export default function Sidebar() {
  ...
  const tp = useTranslations("partnersPage");
    ...
    <>
      ...
      <div className="border-b border-white/10 px-6 py-4 sm:py-5">
        <div className="flex justify-center">
          <div className="flex flex-col items-center">
            ...
            />
            <h1 className="mt-2 text-lg font-semibold tracking-[0.1em] text-white">CLARA OS</h1>
          </div>
        </div>
      </div>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      onClick={() => setOpen(false)}
                      className={`group relative flex items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 transition-all duration-200 ${active ? "border-cyan-400/15 bg-cyan-400/[0.08] text-white shadow-[inset_0_0_20px_rgba(34,211,238,0.03)]" : "text-slate-400 hover:border-white/[0.06] hover:bg-white/[0.04] hover:text-slate-100"}`}
                    >
                      ...
                      <span className="text-[14px] font-medium">{item.key === "partners" ? tp("nav") : t(item.key)}</span>
                    </Link>
                  );
                })}
              </div>
      ...
      <div className="border-t border-white/10 px-5 py-4">
        <div className="mb-4 flex items-center gap-2">
          ...
          <span className="text-sm text-slate-300">{tc("ready")}</span>
        </div>

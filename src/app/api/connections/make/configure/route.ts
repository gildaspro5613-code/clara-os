...
function scenarioMap(value: MakeWebhookCredentials | null): MakeWebhookCredentials["scenarios"] {
  ...
  for (const [key, scenario] of Object.entries(value.scenarios)) {
    ...
    const headerEntries = scenario.headers && typeof scenario.headers === "object" && !Array.isArray(scenario.headers)
      ? Object.entries(scenario.headers).filter(([name, headerValue]) => HEADER_NAME.test(name) && typeof headerValue === "string" && headerValue.length <= 4096 && !/[\r\n]/.test(headerValue)).slice(0, MAX_HEADERS)
      : [];
    ...
    const timeoutMs = typeof scenario.timeoutMs === "number" && Number.isFinite(scenario.timeoutMs)
      ? Math.min(39_000, Math.max(1_000, Math.round(scenario.timeoutMs))) : undefined;
...
export async function POST(request: Request) {
  ...
  try { body = (await request.json()) as ConfigureMakeRequest; }
  catch { return NextResponse.json({ error: "INVALID_REQUEST" }, { status: 400, headers: PRIVATE_HEADERS }); }

  const webhookUrl = normalizeWebhookUrl(body.webhookUrl);
  if (!validScenarioKey(body.scenarioKey) || !webhookUrl) return NextResponse.json({ error: "INVALID_CONFIGURATION" }, { status: 400, headers: PRIVATE_HEADERS });
  ...
  try {
    ...
    if (existing && previousCredentials && Object.keys(previousCredentials.scenarios ?? {}).length !== Object.keys(previousScenarios).length) {
      await repository.updateStatus(connection.id, ConnectionStatus.RECONNECT_REQUIRED).catch(() => undefined);
      return NextResponse.json({ error: "EXISTING_CONFIGURATION_INVALID" }, { status: 409, headers: PRIVATE_HEADERS });
    }
    ...
    const credentials: MakeWebhookCredentials = { scenarios: { ...previousScenarios, [scenarioKey]: { ...(previousScenario?.headers ? { headers: previousScenario.headers } : {}), ...(previousScenario?.timeoutMs ? { timeoutMs: previousScenario.timeoutMs } : {}), url: webhookUrl } } };
    ...
    try { await repository.save({ ...connection, status: ConnectionStatus.CONFIGURED, scopes: [...scopes], updatedAt: now }); }
    catch (error) {

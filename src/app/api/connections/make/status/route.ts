...
export async function GET() {
  try {
    ...
    const scenarioKeys = [...new Set(connection.scopes.filter((scope) => scope.startsWith("make:scenario:")).map((scope) => scope.slice("make:scenario:".length)).filter(validScenarioKey))].sort();
